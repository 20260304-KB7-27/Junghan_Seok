...
-- ColumnA != B : 컬럼의 내용중 B와 같지 않은 데이터만 보여주기 -> 확장문법
-- ColumnA <> B : 컬럼의 내용중 B와 같지 않은 데이터만 보여주기 -> 표준
select
    first_name,
    last_name,
    gender
...
where
gender = 'M' AND
first_name = 'Aamod'; -- 성별이 M이 아닌 데이터만
...
패턴
% : 0개이상의 문자를 나타낸다.
'%apple%' -> apple포함된 모든 문자열을 의미

_ : 1개의 문자를 나타냄
'a_k' 는 'a'로시작하고 'k'로 끝나는 세글자 문자열을 의미
*/
select
    first_name
from
    employees
where
    first_name like 'Aa%'; -- first_name이 Aa 시작하는 모든 데이터
...
-- first_name이 Aa 시작하고 d로끝나는 5글자인 모든 데이터
